...
def get_climate_emissions():
    countries = {
        "China": 11500, "USA": 5000, "India": 2900,
...
def get_renewable_energy():
    data = {
        "Iceland": 85, "Norway": 71, "Sweden": 60,
        "Brazil": 46,  "Germany": 29
    }
    config = {
        "type": "horizontalBar",
        "data": {
        ...
        "options": {
            "title":  _title("⚡ Renewable Energy Share (%)"),
            "legend": _legend(),
...
def get_temperature_trend():
    ...
    temps = [0.70,0.60,0.64,0.66,0.74,0.87,
    ...
    config = {
        ...
        "data": {
            "labels": years,
            "datasets": [{
                "label": "Temperature anomaly vs 1951–1980 baseline (°C)",
                "data":  temps,
                "borderColor":     "#e74c3c",
        ...
        "options": {
            "title":  _title("🌡️ Global Temperature Anomaly 2010–2023"),
            "legend": _legend(),
...
# ── ECONOMICS ─────────────────────────────────────────────────────────────────
def get_gdp_growth():
    """Major economies GDP growth rate — World Bank 2023 estimates."""
    economies = {
        "India": 6.3,  "China": 5.2,    "USA": 2.5,
        "Germany": -0.3,"UK": 0.1,      "Japan": 1.9,
        "Brazil": 2.9, "S.Africa": 0.6
    }
    colors = ["#2ecc71" if v >= 0 else "#e74c3c" for v in economies.values()]
    config = {
        "type": "bar",
        "data": {
            "labels": list(economies.keys()),
            "datasets": [{
                "label": "GDP Growth Rate (%) — World Bank 2023",
                "data":  list(economies.values()),
                "backgroundColor": colors
            }]
        },
        "options": {
            "title":  _title("📈 GDP Growth Rate — Major Economies (2023)"),
            "legend": _legend(),
            "scales": _axes(y_label="Growth Rate (%)", y_min=-1.5, y_max=7.5)
        }
    }
    return make_chart(config, 560, 320)

def get_inflation_chart():
    """Global inflation rates 2023 — IMF data."""
    countries = {
        "Argentina": 133, "Turkey": 64,  "Nigeria": 28,
        "Brazil": 5.1,    "USA": 3.4,    "EU": 5.4,
        "China": 0.2,     "Japan": 3.3
    }
    ...
    config = {
        ...
        "data": {
            "labels": list(countries.keys()),
            "datasets": [{
                "label": "Inflation Rate (%) — IMF 2023",
                "data":  list(countries.values()),
                "backgroundColor": colors
            }]
        },
        "options": {
            "title":  _title("💸 Inflation Rates 2023 (IMF)"),
            "legend": _legend(),
...
def get_trade_balance():
    """Trade balance — exports minus imports, USD billions, 2023 est."""
    countries = {
        "China": 823,  "Germany": 224, "Japan": -9,
        "USA": -778,   "UK": -232,     "India": -247
    }
    colors = ["#2ecc71" if v >= 0 else "#e74c3c" for v in countries.values()]
    config = {
        "type": "bar",
        "data": {
            "labels": list(countries.keys()),
            "datasets": [{
                "label": "Trade Balance (USD Billion, 2023 est.)",
                "data":  list(countries.values()),
                "backgroundColor": colors
            }]
        },
        "options": {
            "title":  _title("⚖️ Trade Balance — Major Economies (2023)"),
            "legend": _legend(),
